    ...
    internal class TransitionTree {
        ...
        public void Add(char c, bool lowerCase, DFAState state) {
            ...
            if (value == '\0') {
                value = c;
                state = state;
                left = new TransitionTree();
                right = new TransitionTree();
            } else if (value > c) {
        ...
        public void PrintTo(StringBuilder buffer, String indent) {
            ...
            if (value != '\0') {
                ...
                if (state.value != null) {
                    buffer.Append(": ");
                    buffer.Append(state.value);
                    buffer.Append("\n");
                }
                state.tree.PrintTo(buffer, indent + " ");
            }
            if (right != null) {
                right.PrintTo(buffer, indent);
            }
        }
    }
}
