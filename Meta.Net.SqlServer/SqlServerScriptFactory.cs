...
using Meta.Net.SqlServer.SqlServerModuleParser;
using Meta.Net.Sync;
...
namespace Meta.Net.SqlServer
{
    public class SqlServerScriptFactory : DataScriptFactory
    {
        ...
        public override string CreateForeignKey(DataContext sourceDataContext, DataContext targetDataContext, ForeignKey foreignKey)
        {
            ...
            foreach (var foreignKeyColumn in foreignKey.ForeignKeyColumns)
            {
                if (first)
                {
                    ...
                    builder2.Append(foreignKey.ReferencedUserTable.Schema.ObjectName);
                    builder2.Append("].[");
		...
		// TODO: Figure out usage of source and target data contexts.
