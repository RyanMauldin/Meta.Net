...
namespace Meta.Net.Data
{
    ...
    public abstract class DataSyncManager : ISyncManager, IScriptable
    {
            ...
            try
            {
                ...
                if (DataProperties.CatalogsToCompare.Count > 0)
                {
                    ...
                    foreach (var catalogKeyValuePair in
                        ...
                        if (!Server.RenameCatalog(DataSyncComparer.SourceServer, catalogKeyValuePair.Key, catalogKeyValuePair.Value + "<~>Renamed"))
                            Exceptions.Add(new Exception(
                                "DataSyncManager.CompareForSync() method threw an exception" +
                                " where DataProperties.CatalogsToCompare was not initialized appropriately. Ensure" +
                                " no catalogs on the source is mismatched for compare on target if using DataProperties.CatalogsToCompare member."));
                    ...
                    foreach (var catalogKeyValuePair in
                        ...
                        if (!Server.RenameCatalog(DataSyncComparer.SourceServer, catalogKeyValuePair.Value + "<~>Renamed", catalogKeyValuePair.Value))
                            Exceptions.Add(new Exception(
                                "DataSyncManager.CompareForSync() method threw an exception" +
                                " where DataProperties.CatalogsToCompare was not initialized appropriately. Ensure" +
                                " no catalogs on the source is mismatched for compare on target if using DataProperties.CatalogsToCompare member."));
                ...
                if (DataProperties.CatalogsToCompare.Count > 0)
                {
                    ...
                    foreach (var catalogKeyValuePair in
                        ...
                        if (!Server.RenameCatalog(DataSyncComparer.SourceServer, catalogKeyValuePair.Value, catalogKeyValuePair.Key + "<~>Renamed"))
                            Exceptions.Add(new Exception(
                                "DataSyncManager.CompareForSync() method threw an exception" +
                                " where DataProperties.CatalogsToCompare was not initialized appropriately. Ensure" +
                                " no catalogs on the source is mismatched for compare on target if using DataProperties.CatalogsToCompare member."));
                    ...
                    foreach (var catalogKeyValuePair in
                        ...
                        if (!Server.RenameCatalog(DataSyncComparer.SourceServer, catalogKeyValuePair.Key + "<~>Renamed", catalogKeyValuePair.Key))
                            Exceptions.Add(new Exception(
                                "DataSyncManager.CompareForSync() method threw an exception" +
                                " where DataProperties.CatalogsToCompare was not initialized appropriately. Ensure" +
                                " no catalogs on the source is mismatched for compare on target if using DataProperties.CatalogsToCompare member."));
        ...
        public bool FetchSourceDataModelFromServer(bool closeConnectionOnFill = true)
        {
            ...
            Server.Fill(SourceServer, SourceDataConnectionManager);
        ...
        public bool FetchTargetDataModelFromServer(bool closeConnectionOnFill = true)
        {
            ...
            Server.Fill(TargetServer, TargetDataConnectionManager);

            if (closeConnectionOnFill)
                TargetDataConnectionManager.Close();
        ...
        public UserTable FindUserTable(DataContext sourceDataContext, DataContext targetDataContext, string nameSpace, Server server)
        {
            ...
            var tableParts = nameSpace
                .Replace(sourceDataContext.IdentifierOpenChar, "")
                .Replace(sourceDataContext.IdentifierCloseChar, "")
                .Replace(targetDataContext.IdentifierOpenChar, "")
                .Replace(targetDataContext.IdentifierCloseChar, "")
                .Split('.');

            if (tableParts.Length < 2 || tableParts.Length > 3)
                throw new Exception(string.Format("DataProperties.TablesForSync must include Catalog, Schema(optional - Can use dbo for MySql as default even though this doesn't truely exist for MySql), and User-Table for user-table namespace {0}. Do not use the \".\" character in user-table names or an improper match will occur! If you need to use the \".\" character in your table names let us know in our forums!", nameSpace));

            Catalog catalog;
            if (!server.Catalogs.TryGetValue(tableParts[0], out catalog))
                throw new Exception(string.Format("Catalog {0} did not exist for user-table namespace {1}.", tableParts[0], nameSpace));
            // Default the table name to the first part without the schema name

            if (tableParts.Length == 2)
            {
                foreach (var possibleSchema in catalog.Schemas.Values)
                {
                    UserTable possibleUserTable;
                    if (!possibleSchema.UserTables.TryGetValue(tableParts[1], out possibleUserTable))
                        continue;

                    return possibleUserTable;
                }

                throw new Exception(string.Format("User-Table {0} did not exist for user-table namespace {1}.", tableParts[2], nameSpace));
            }

            Schema schema;
            if (!catalog.Schemas.TryGetValue(tableParts[1], out schema))
            {
                if (server.DataContext.IncludeSchemaNameInNamespace)
                    throw new Exception(string.Format("Schema {0} did not exist for user-table namespace {1}.", tableParts[1], nameSpace));

                foreach (var possibleSchema in catalog.Schemas.Values)
                {
                    UserTable possibleUserTable;
                    if (!possibleSchema.UserTables.TryGetValue(tableParts[2], out possibleUserTable))
                        continue;
            ...
            UserTable userTable;
            if (!schema.UserTables.TryGetValue(tableParts[2], out userTable))
            {
                if (server.DataContext.IncludeSchemaNameInNamespace)
                    throw new Exception(string.Format("Schema {0} did not exist for user-table namespace {1}.", tableParts[1], nameSpace));
