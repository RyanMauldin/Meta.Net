...
using Meta.Net.Data.SqlServer.SqlServerModuleParser;
using System.Text;

namespace Meta.Net.Data.SqlServer
{
    public class SqlServerScriptFactory : DataScriptFactory
    {
		#region Methods (57) 
        ...
        public override string AlterAggregateFunction(DataContext sourceDataContext, DataContext targetDataContext, AggregateFunction aggregateFunction)
        {
            return sourceDataContext.ContextType != targetDataContext.ContextType
                ? null
                : SqlServerModuleManager.GetAlterDefinition(aggregateFunction.Definition);
        }
        ...
        {
            // TODO: Calling method will also need to check for dropping this check constraint
            ...
            if (!(DataProperties.ReplicateItemsMarkedNotForReplication || !checkConstraint.IsNotForReplication))
                return string.Empty;

            var builder = new StringBuilder();
            builder.Append("ALTER TABLE ");
            builder.Append(CreateIdentifier(checkConstraint.UserTable));
            builder.Append(checkConstraint.IsDisabled ? " NOCHECK CONSTRAINT [" : " CHECK CONSTRAINT [");
            builder.Append(checkConstraint.ObjectName);
            builder.Append("]");
            return builder.ToString();
        }
        ...
        {
            // TODO: Calling method will also need to check for dropping this foreign key
            // TODO: Add check for DataProperties.DropItemsInTargetOnNotForReplication and handle
            // TODO: that accordingly for CheckConstraints, Identity Columns, Triggers, and Foreign Keys.
        ...
        public override string AlterInlineTableValuedFunction(DataContext sourceDataContext, DataContext targetDataContext, InlineTableValuedFunction inlineTableValuedFunction)
        {
            return sourceDataContext.ContextType != targetDataContext.ContextType
                ? null
                : SqlServerModuleManager.GetAlterDefinition(inlineTableValuedFunction.Definition);
        }
        ...
        public override string CreateCheckConstraint(DataContext sourceDataContext, DataContext targetDataContext, CheckConstraint checkConstraint)
        {
            ...
            builder.Append(CreateIdentifier(checkConstraint.UserTable));
            builder.Append(" WITH ");
            builder.Append(checkConstraint.IsNotTrusted ? "NOCHECK" : "CHECK");
            builder.Append(" ADD CONSTRAINT [");
            builder.Append(checkConstraint.ObjectName);
            builder.Append("] CHECK ");
            builder.Append(checkConstraint.Definition);
            return builder.ToString();
        }
        ...
        public override string CreateForeignKey(DataContext sourceDataContext, DataContext targetDataContext, ForeignKey foreignKey)
        {
            ...
            builder.Append(CreateIdentifier(foreignKey.UserTable));
            builder.Append(" WITH ");
            builder.Append(foreignKey.IsNotTrusted ? "NOCHECK " : "CHECK ");
            ...
            foreach (var foreignKeyColumn in foreignKey.ForeignKeyColumns.Values)
            {
                if (first)
                {
                    ...
                    builder2.Append(foreignKey.ReferencedSchemaName);
                    builder2.Append("].[");
                    builder2.Append(foreignKey.ReferencedTableName);
                    builder2.Append("] ([");
                    builder2.Append(foreignKeyColumn.ReferencedColumnName);
                    builder2.Append("]");
        ...
        {
            return string.Format("[{0}]", string.Join("].[", obj.Split(".".ToCharArray(), StringSplitOptions.None)));
        }
        ...
        public override string CreateIndex(DataContext sourceDataContext, DataContext targetDataContext, Index index)
        {
            ...
            // TODO: Figure out what to do for isDisabled
            // TODO: http://msdn.microsoft.com/en-us/library/ms188388.aspx
        ...
        {
            if (DataProperties.NonRemovableSchemas.Contains(CreateIdentifier(schema)))
                return null;

            var builder = new StringBuilder();
            builder.Append("CREATE SCHEMA ");
            builder.Append(CreateIdentifier(schema));
            return builder.ToString();
        }
        ...
        public override string CreateUserDefinedDataType(DataContext sourceDataContext, DataContext targetDataContext, UserDefinedDataType userDefinedDataType)
        {
            ...
            builder.Append(" FROM [");
            builder.Append(userDefinedDataType.DataType);
            builder.Append("]");
            builder.Append(GetColumnPrecision(sourceDataContext, targetDataContext, userDefinedDataType));
            builder.Append(userDefinedDataType.IsNullable ? " NULL" : " NOT NULL");
            return builder.ToString();
        }
        ...
        public override string InsertRow(DataContext sourceDataContext, DataContext targetDataContext, UserTable userTable, object[] values)
        {
            ...
            var i = 0;
            foreach (var column in userTable.UserTableColumns.Values)
            {
                var isLast = column == userTable.UserTableColumns.Values.Last();
        ...
        private static string GetColumnPrecision(DataContext sourceDataContext, DataContext targetDataContext, UserDefinedDataType userDefinedDataType)
        {
            var builder = new StringBuilder();

            switch (userDefinedDataType.DataType)
            {
                case "decimal":
                case "numeric":
                    if (userDefinedDataType.Precision > 0 && userDefinedDataType.Scale > 0)
                    {
                        builder.Append("(");
                        builder.Append(userDefinedDataType.Precision);
                        builder.Append(",");
                        builder.Append(userDefinedDataType.Scale);
                        builder.Append(")");
                        return builder.ToString();
                    }
                ...
                case "real":
                case "datetimeoffset":
                    if (userDefinedDataType.Precision > 0)
                    {
                        builder.Append("(");
                        builder.Append(userDefinedDataType.Precision);
                        builder.Append(")");
                ...
                case "nchar":
                case "binary":
                    if (userDefinedDataType.MaxLength > 0)
                    {
                        builder.Append("(");
                        builder.Append(userDefinedDataType.MaxLength);
                        builder.Append(")");
                        return builder.ToString();
                    }
